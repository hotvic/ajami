...
static const char* out_names[BCHANNELS] = {
    "a.master.out_L", "a.master.out_R", "b.low.out_L",  "b.low.out_R",
    "c.mid.out_L",    "c.mid.out_R",    "d.high.out_L", "d.high.out_R"};
static const char* iports[NCHANNELS] = {NULL, NULL};
static const char* oports[BCHANNELS] = {NULL, NULL, NULL, NULL,
                                        NULL, NULL, NULL, NULL};
    ...
    do {
        if (tr_buf[t].message[0] != '\0')
            fprintf(stderr, "%s trace [%" PRIu32 "]: %s\n",
                    g_get_application_name(), tr_buf[t].timestamp,
                    tr_buf[t].message);
        t = (t + 1) & (TR_BUFSIZE - 1);
    } while (t != tr_next);
...
void io_errlog(int err, char* fmt, ...) {
    ...
    IF_DEBUG(DBG_TERSE, io_trace("error %d: %s", err, buffer));
    g_print(_("%s internal error %d: %s\n"), g_get_application_name(), err,
            buffer);
    if (all_errors_fatal) {
...
void io_set_latency(int source, jack_nframes_t delay) {
    ...
    static char* latency_sources[LAT_NSOURCES] = {
        "I/O Buffering", "Fourier Transform", "Limiter"};
    int chan;
    ...
    IF_DEBUG(DBG_TERSE, io_trace("latency due to %s is %ld frames.",
...
int io_process(jack_nframes_t nframes, void* arg) {
    ...
    if (nframes < dsp_block_size) {
        ...
        if (have_dsp_thread)
            ...
            return_code =
                process_signal(nframes, nchannels, bchannels, in, out);
        ...
        while (nframes >= dsp_block_size) {
            ...
            if ((rc = process_signal(dsp_block_size, nchannels, bchannels, in,
                                     out)) != 0)
                return_code = rc;
...
int io_bufsize(jack_nframes_t nframes, void* arg) {
    ...
    io_set_latency(
        LAT_BUFFERS,
        (have_dsp_thread && (dsp_block_size > nframes) ? dsp_block_size : 0));
    return 0;
}
...
gboolean check_file(char* optarg) {
    ...
    if ((fp = fopen(optarg, "r")) == NULL) {
        errstr = g_strdup_printf(_("File %s : %s\nUsing default."), optarg,
                                 strerror(errno));
        g_print("%s\n", errstr);
        ajami_message(GTK_MESSAGE_ERROR, errstr);
        free(errstr);
        return (FALSE);
    }
...
jack_client_t* io_jack_open() {
    ...
    if (server_name) {
        client =
            jack_client_open(client_name, JackServerName, &status, server_name);
    } else {
...
void io_init(AjamiArgs* args) {
    ...
    if (spectrum_freq < 0 || spectrum_freq > 10)
        spectrum_freq = 10;
    ...
    if (ajami_args_daemon_mode)
        gui_mode = 2;

    if (ajami_args_limiter == 1)
    ...
    */
    ...
    if (connect_ports) {
        ...
        /* TODO: support port names from command line
    ...
    if (!client_name) {
        client_name = strdup(g_get_application_name());
    }
...
int io_create_dsp_thread() {
    ...
    rc = pthread_getschedparam(jack_client_thread_id(client), &policy,
                               &rt_param);
    if (rc) {
    ...
    if (jst.realtime) {
        IF_DEBUG(DBG_TERSE, io_trace("JACK realtime priority = %d",
                                     rt_param.sched_priority));
        rt_param.sched_priority -= DSP_PRIORITY_DIFF;
        IF_DEBUG(DBG_TERSE, io_trace("DSP realtime priority = %d",
                                     rt_param.sched_priority));
    } else
    ...
    rc = jack_create_thread(&dsp_thread, rt_param.sched_priority, jst.realtime,
                            io_dsp_thread, NULL);
    ...
    */
    ...
    if ((policy = sched_getscheduler(0)) < 0) {
        io_errlog(EPERM, "Cannot get current client scheduler: %s",
                  strerror(errno));
        return -1;
    }
    ...
    if (sched_getparam(0, &my_param)) {
        io_errlog(EPERM, "Cannot get current client scheduler parameters: %s",
                  strerror(errno));
        return -1;
    }
...
void io_activate() {
    ...
    for (chan = 0; chan < nchannels; chan++) {
        ...
        input_ports[chan] =
            jack_port_register(client, in_names[chan], JACK_DEFAULT_AUDIO_TYPE,
                               JackPortIsInput, 0);

        if (input_ports[chan] == NULL) {
            g_print(_("%s: Cannot register JACK ports."),
                    g_get_application_name());
            exit(2);
        }
    }
    ...
    for (chan = 0; chan < bchannels; chan++) {
        output_ports[chan] =
            jack_port_register(client, out_names[chan], JACK_DEFAULT_AUDIO_TYPE,
                               JackPortIsOutput, 0);
    ...
    if (jack_activate(client)) {
        g_print(_("%s: Cannot activate JACK client."),
                g_get_application_name());
        exit(2);
    }
    ...
    io_set_latency(LAT_BUFFERS,
                   (have_dsp_thread && (dsp_block_size > jst.buf_size)
                        ? dsp_block_size
                        : 0));
    pthread_mutex_unlock(&lock_dsp);
}
