...
static GtkSpinButton *l_hdeq_lower_gain, *l_hdeq_upper_gain, *l_crossfade_time,
    *l_inmeter_warn, *l_spectrum_freq, *l_rms_time_slice, *l_xo_delay_time_low,
    *l_xo_delay_time_mid;
...
/*
  This function opens and reads a file called jamin-defaults.  We're only
  writing
...
void preferences_init() {
    ...
    // l_SpectrumComboBox = GTK_COMBO_BOX (lookup_widget (pref_dialog,
    // "SpectrumComboBox"));
    // gtk_combo_box_set_active (l_SpectrumComboBox, process_get_spec_mode ());

    // l_ColorsComboBox = GTK_COMBO_BOX (lookup_widget (pref_dialog,
    // "ColorsComboBox"));
    // gtk_combo_box_set_active (l_ColorsComboBox, LOW_BAND_COLOR);

    // l_limiter_combo = GTK_COMBO_BOX (lookup_widget (pref_dialog,
    // "limiter_combo"));

    // gtk_combo_box_set_active (l_limiter_combo, process_get_limiter_plugin
    // ());

    // l_hdeq_lower_gain = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "MinGainSpin"));
    // l_hdeq_upper_gain = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "MaxGainSpin"));
    // l_crossfade_time = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "CrossfadeTimeSpin"));
    // l_inmeter_warn = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "warningLevelSpinButton"));
    // l_spectrum_freq = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "UpdateFrequencySpin"));
    // l_rms_time_slice = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "rmsTimeValue"));
    // l_xo_delay_time_low = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "LowDelaySpinButton"));
    // l_xo_delay_time_mid = GTK_SPIN_BUTTON (lookup_widget (pref_dialog,
    // "MidDelaySpinButton"));

    // l_out_meter_peak = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog,
    // "out_meter_peak_button"));
    // l_out_meter_full = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog,
    // "out_meter_full_button"));
    // l_rms_meter_peak = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog,
    // "rms_meter_peak_button"));
    // l_rms_meter_full = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog,
    // "rms_meter_full_button"));
    // l_fft = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog, "FFTButton"));
    // l_iir = GTK_TOGGLE_BUTTON (lookup_widget (pref_dialog, "IIRButton"));

    // color_dialog = create_colorchooserdialog1 (pref_dialog);
    ...
    // g_signal_connect (
    // gtk_widget_get_property(color_dialog,'ok-button',&ok_button1),
    //"clicked", G_CALLBACK (color_ok_callback), color_dialog);

    // g_signal_connect (
    // gtk_widget_get_property(color_dialog,'cancel-button',&cancel_button1),
    //"clicked", G_CALLBACK (color_cancel_callback), color_dialog);

    // g_signal_connect (
    // gtk_widget_get_property(color_dialog,'help-button',&help_button1),
    ...
    // if (process_limiter_plugins_available () < 2) gtk_widget_set_sensitive
    // (GTK_WIDGET (l_limiter_combo), FALSE);
...
void pref_set_all_values() {
    ...
    g_signal_handlers_block_by_func (l_out_meter_full,
    on_out_meter_full_button_clicked, NULL);
    g_signal_handlers_block_by_func (l_rms_meter_peak,
    on_rms_meter_peak_button_clicked, NULL);
    g_signal_handlers_block_by_func (l_rms_meter_full,
    on_rms_meter_full_button_clicked, NULL);
    ...
    gtk_toggle_button_set_active(l_out_meter_full,
                                 !intrim_get_out_meter_peak_pref());
    gtk_toggle_button_set_active(l_rms_meter_peak,
    ...
    on_out_meter_peak_button_clicked, NULL);
    g_signal_handlers_unblock_by_func (l_out_meter_full,
    on_out_meter_full_button_clicked, NULL);
    g_signal_handlers_unblock_by_func (l_rms_meter_peak,
    on_rms_meter_peak_button_clicked, NULL);
    g_signal_handlers_unblock_by_func (l_rms_meter_full,
    on_rms_meter_full_button_clicked, NULL);
...
void pref_write_jamin_defaults() {
    ...
    if ((fp = fopen(file, "w")) != NULL) {
        ...
        fprintf(fp, "[LOW BAND COMPRESSOR COLOR]  = %f %f %f\n",
                color[LOW_BAND_COLOR].red, color[LOW_BAND_COLOR].green,
                color[LOW_BAND_COLOR].blue);

        fprintf(fp, "[MID BAND COMPRESSOR COLOR]  = %f %f %f\n",
                color[MID_BAND_COLOR].red, color[MID_BAND_COLOR].green,
                color[MID_BAND_COLOR].blue);

        fprintf(fp, "[HIGH BAND COMPRESSOR COLOR] = %f %f %f\n",
                color[HIGH_BAND_COLOR].red, color[HIGH_BAND_COLOR].green,
        ...
        fprintf(fp, "[PARAMETRIC HANDLE COLOR]    = %f %f %f\n",
                color[HANDLE_COLOR].red, color[HANDLE_COLOR].green,
                color[HANDLE_COLOR].blue);

        fprintf(fp, "[HDEQ CURVE COLOR]           = %f %f %f\n",
                color[HDEQ_CURVE_COLOR].red, color[HDEQ_CURVE_COLOR].green,
        ...
        fprintf(fp, "[HDEQ GRID COLOR]            = %f %f %f\n",
                color[HDEQ_GRID_COLOR].red, color[HDEQ_GRID_COLOR].green,
        ...
        fprintf(fp, "[TEXT COLOR]                 = %f %f %f\n",
                color[TEXT_COLOR].red, color[TEXT_COLOR].green,
                color[TEXT_COLOR].blue);

        fprintf(fp, "[METER NORMAL COLOR]         = %f %f %f\n",
                color[METER_NORMAL_COLOR].red, color[METER_NORMAL_COLOR].green,
        ...
        fprintf(fp, "[METER OVER COLOR]           = %f %f %f\n",
                color[METER_OVER_COLOR].red, color[METER_OVER_COLOR].green,
                color[METER_OVER_COLOR].blue);

        fprintf(fp, "[METER PEAK COLOR]           = %f %f %f\n",
                color[METER_PEAK_COLOR].red, color[METER_PEAK_COLOR].green,
