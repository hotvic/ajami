...
static HVMeter *le_meter[XO_BANDS], *ga_meter[XO_BANDS];
...
void th_changed(int id, float value) {
  ...
  if (!suspend_gang && gang_th[i]) {
    ...
    for (j = 0; j < XO_BANDS; j++) {
      if (i != j && gang_th[j]) {
        ...
        hv_meter_set_warn_point(le_meter[j], new_value);
      }
    }
  }
  ...
  hv_meter_set_warn_point(le_meter[i], value);
}
...
comp_settings comp_get_settings(int band) { return (compressors[band]); }
...
void suspend_ganging() { suspend_gang = TRUE; }
