...
void geq_set_gains() {
    ...
    if (!EQ_drawn) {
        ...
        for (bin = 1; bin < (BINS / 2 - 1); bin++) {
            eq_coefs[bin] =
                ((1.0f - bin_delta[bin]) * geq_gains[bin_base[bin]]) +
...
void geq_set_coefs(int length, float x[], float y[]) {
    ...
    if (length < BINS / 2 - 1) {
        errstr = g_strdup_printf(
            _("Splined length %d does not match BINS / 2 - 1 (%d)"), length,
            BINS / 2 - 1);
...
void geq_set_sliders(int length, float x[], float y[]) {
    ...
    if (length < BINS / 2 - 1) {
        ...
        for (j = 1, i = 0; j < EQ_BANDS - 1; j++) {
            while (geq_freqs[j] > x[i])
                i++;

            value = y[i - 1] +
                    (y[i] - y[i - 1]) *
                        ((geq_freqs[j] - x[i - 1]) / (x[i] - x[i - 1]));
...
GtkAdjustment* geq_get_adjustment(int band) {
    if (band < 0 || band > EQ_BANDS) {
        fprintf(
            stderr,
            _("jam error: Adjustment from out-of-range band %d requested\n"),
            band);
        exit(1);
    }
    return geqa[band];
