...
static AjamiCompressor* l_comp[3];
static GtkLabel *l_EQ_curve_lbl = NULL, *l_c_curve_lbl[3];
...
static float EQ_curve_range_x, EQ_curve_range_y, EQ_curve_width,
    ...
    EQ_yinterp[EQ_INTERP + 1],
        *EQ_xinput = NULL, *EQ_yinput = NULL, l_geq_freqs[EQ_BANDS],
        l_geq_gains[EQ_BANDS], comp_curve_range_x[3], comp_curve_range_y[3],
        comp_curve_width[3], comp_curve_height[3], comp_start_x[3],
        comp_start_y[3], comp_end_x[3], comp_end_y[3],
        EQ_freq_xinterp[EQ_INTERP + 1], EQ_freq_yinterp[EQ_INTERP + 1],
...
/*  Note the array dimensions for the parametric notch handles.  Here's how
    they're laid out - first dimension [2] = X and Y indexes; second dimension
   [3] =
    left handle, center handle, right handle (left shelf 0 is 0, right shelf 2
   is
    EQ_CURVE_width); third dimension is notch number from left (0).  */

static int EQ_mod = 1, EQ_drawing = 0, EQ_input_points = 0, EQ_length = 0,
           EQ_draw_dir = 0, comp_realized[3] = {0, 0, 0}, EQ_realized = 0,
           xover_active = 0, xover_handle_l2m, xover_handle_m2h,
           EQ_drag_l2m = 0, EQ_drag_m2h = 0, EQ_notch_drag[NOTCHES],
           EQ_notch_Q_drag[NOTCHES], EQ_notch_handle[2][3][NOTCHES],
           EQ_notch_width[NOTCHES], EQ_notch_index[NOTCHES],
           EQ_notch_flag[NOTCHES];
...
void bind_hdeq() {
    ...
    /* TODO: what are these labels on UI ?
...
float hdeq_get_notch_default_freq(int i) { return (EQ_notch_default[i]); }
...
void hdeq_low2mid_set(GtkRange* range) {
    ...
    if (value == gtk_adjustment_get_lower(l_low2mid_adj)) {
        ajami_compressor_set_disabled(l_comp[0], TRUE);
    } else {
        ajami_compressor_set_disabled(l_comp[0], FALSE);
    }
    ...
    label =
        g_strdup_printf(_("<b>Mid : %d - %d</b>"), NINT(lvalue), NINT(hvalue));
    ajami_compressor_set_text(l_comp[1], label);
    free(label);

    lvalue = pow(10.0, gtk_adjustment_get_lower(l_low2mid_adj));
    mvalue = pow(10.0, value);
    label =
        g_strdup_printf(_("<b>Low : %d - %d</b>"), NINT(lvalue), NINT(mvalue));
    ajami_compressor_set_text(l_comp[0], label);
    free(label);
...
void hdeq_mid2high_set(GtkRange* range) {
    ...
    if (value == gtk_adjustment_get_upper(l_low2mid_adj)) {
        ajami_compressor_set_disabled(l_comp[2], TRUE);
    } else {
        ajami_compressor_set_disabled(l_comp[2], FALSE);
    }
    ...
    label =
        g_strdup_printf(_("<b>Mid : %d - %d</b>"), NINT(lvalue), NINT(mvalue));
    ajami_compressor_set_text(l_comp[1], label);
    free(label);

    hvalue = pow(10.0, gtk_adjustment_get_upper(l_low2mid_adj));
    label =
        g_strdup_printf(_("<b>High : %d - %d</b>"), NINT(mvalue), NINT(hvalue));
    ajami_compressor_set_text(l_comp[2], label);
    free(label);
...
static void logfreq2xpix(float log_freq, int* x) {
    *x = NINT(((log_freq - gtk_adjustment_get_lower(l_low2mid_adj)) /
               EQ_curve_range_x) *
              EQ_curve_width);

    //      printf("%f, %f, %i, %i\n",log_freq, gtk_adjustment_get_lower
    //      (l_low2mid_adj),  EQ_curve_range_x,
    //      EQ_curve_width );
}
...
static void gain2ypix(float gain, int* y) {
    *y = EQ_curve_height -
         NINT(((gain - EQ_gain_lower) / EQ_curve_range_y) * EQ_curve_height);
}
...
void draw_EQ_spectrum_curve(float single_levels[]) {
    ...
    if (!EQ_drawing && !xover_active) {
        ...
        for (i = 0; i < EQ_INTERP; i++) {
            ...
            spectrum_y[i] =
                NINT(-(lin2db(single_levels[bin]) / EQ_SPECTRUM_RANGE) *
                     EQ_curve_height);
        }
...
static void set_EQ() {
    ...
    interpolate(interval, EQ_length, l_geq_freqs[0], l_geq_freqs[EQ_BANDS - 1],
                &EQ_length, x, EQ_y_notched, EQ_freq_xinterp, EQ_freq_yinterp);
    ...
    for (j = 0; j < NOTCHES; j++) {
        ...
        if (EQ_notch_flag[j]) {
            ...
            if (!j) {
                ...
                interpolate(EQ_interval, 4, x[0], x[3], &length, x, y,
                            &EQ_x_notched[slide], &EQ_y_notched[slide]);
            }
            ...
            /*  This is the high-shelving notch.  */
                ...
                interpolate(EQ_interval, 4, x[0], x[3], &length, x, y,
                            &EQ_x_notched[ndx], &EQ_y_notched[ndx]);
            }
            ...
            /*  The "normal" notches.  */
                ...
                interpolate(EQ_interval, 5, x[0], x[4], &length, x, y,
                            &EQ_x_notched[left], &EQ_y_notched[left]);
            }
        }
    }
}
...
void draw_EQ_curve(cairo_t* EQ_cr) {
    ...
    cairo_rectangle(EQ_cr, x1 - XOVER_HANDLE_HALF_SIZE, 0, XOVER_HANDLE_SIZE,
                    XOVER_HANDLE_SIZE);
    cairo_fill(EQ_cr);

    cairo_rectangle(EQ_cr, x1 - XOVER_HANDLE_HALF_SIZE,
    ...
    if (EQ_mod) {
        ...
        interpolate(EQ_interval, EQ_BANDS, EQ_start, EQ_end, &EQ_length, x, y,
                    EQ_xinterp, EQ_yinterp);
    ...
    for (i = 0; i < NOTCHES; i++) {
        ...
        if (i && i < NOTCHES - 1) {
            ...
            cairo_rectangle(EQ_cr, x1 - NOTCH_HANDLE_HALF_WIDTH / 2,
                            y1 - NOTCH_CENTER_HALF_HEIGHT / 3,
                            NOTCH_HANDLE_WIDTH / 2, NOTCH_CENTER_HEIGHT / 3);
            cairo_fill_preserve(EQ_cr);
            color = get_color(HIGH_BAND_COLOR);
            cairo_set_source_rgb(EQ_cr, color->red, color->green, color->blue);
            cairo_set_line_width(EQ_cr, 1.0);
    ...
    /*  if (EQ_drawing){
     ...
     //                                  NINT (EQ_yinput[EQ_input_points - 1]),
     x,
     y);
        }
    */
...
void hdeq_curve_draw(GtkWidget* widget, cairo_t* cr, gpointer data) {
    ...
    EQ_curve_range_x = gtk_adjustment_get_upper(l_low2mid_adj) -
                       gtk_adjustment_get_lower(l_low2mid_adj);

    //  printf("%f, %f\n", gtk_adjustment_get_upper (l_low2mid_adj),
    //  gtk_adjustment_get_lower (l_low2mid_adj));
...
/*  notch is the notch number (0-4), new is the nearest frequency index to the
   cursor,
...
static int check_notch(int notch, int new, int q) {
    ...
    /*  Notches.  */
        ...
        if (q == 1) {
            ...
            if (EQ_notch_index[notch] - left < 5)
                ret = 0;
        }
        ...
        /*  Right handle  */
            ...
            if (right - EQ_notch_index[notch] < 5)
                ret = 0;
        }
...
void hdeq_curve_motion(GdkEventMotion* event) {
    ...
    int i, j, x, y, size, diffx_l2m, diffx_m2h, diff_notch[2], cursor, drag,
        notch_flag = -1, lo, hi, clock_diff;
    ...
    /*  Unset the drawing flag.  If we modify the background in any way we set
       this flag so
        that we can paint the pixmap back to the screen at the end of this
       function.  We
        will also paint the last spectrum curve to the widget itself so that the
       user can
    ...
    if (clock_diff < MOTION_CLOCK_DIFF)
        return;
    ...
    if (x != prev_x || y != prev_y) {
        ...
        freq = pow(10.0,
                   (gtk_adjustment_get_lower(l_low2mid_adj) +
                    (((double)x / (double)EQ_curve_width) * EQ_curve_range_x)));

        gain =
            ((((double)EQ_curve_height - (double)y) / (double)EQ_curve_height) *
             EQ_curve_range_y) +
            EQ_gain_lower;

        s_gain =
            -(EQ_SPECTRUM_RANGE - (((((double)EQ_curve_height - (double)y) /
                                     (double)EQ_curve_height) *
                                    EQ_SPECTRUM_RANGE)));

        coords = g_strdup_printf(_("%dHz , EQ : %.1fdb , Spectrum : %ddb"),
                                 NINT(freq), gain, NINT(s_gain));
        ...
        if (EQ_drawing && EQ_input_points) {
            ...
            if (x != EQ_xinput[EQ_input_points - 1]) {
                ...
                if ((EQ_draw_dir == 1 && x > EQ_xinput[EQ_input_points - 1]) ||
                    ...
                    //                   gdk_gc_set_foreground (EQ_gc, get_color
                    //                   (HDEQ_CURVE_COLOR));
                    //      color = get_color (HDEQ_CURVE_COLOR);
                    //  cairo_set_source_rgb (EQ_cr,color->red,color->green,
                    //  color->blue);
                    ...
                    //                   gdk_draw_line (hdeq_pixmap, EQ_gc,
                    //                                  NINT
                    //                                  (EQ_xinput[EQ_input_points
                    //                                  -
                    //                                  1]),
                    //                                  NINT
                    //                                  (EQ_yinput[EQ_input_points
                    //                                  -
                    //                                  1]), x, y);
                    //                   gdk_gc_set_foreground (EQ_gc, get_color
                    //                   (TEXT_COLOR));
        ...
        /*  We're just moving the cursor in the window or we're dragging the
                ...
                if (EQ_notch_drag[i]) {
                    ...
                    if (event->state & GDK_SHIFT_MASK) {
                        if (y >= 0 && y <= EQ_curve_height) {
                            EQ_notch_gain[i] =
                                (((((double)EQ_curve_height - (double)y) /
                                   (double)EQ_curve_height) *
                                  EQ_curve_range_y) +
                                 EQ_gain_lower) *
                                0.05;
            ...
            if (notch_flag != -1) {
                ...
                coords = g_strdup_printf(_("%ddb , %dHz - %dHz"), NINT(gain),
                                         lo, hi);
                gtk_label_set_text(l_EQ_curve_lbl, coords);
                free(coords);
            }
        ...
        /*  If we drew anything, blast the background pixmap to the widget and
           paint
           the last
        ...
        if (modified) {
            ...
            //               for (i = 0 ; i < EQ_INTERP ; i++)
            //                {
            //                  if (i) gdk_draw_line (EQ_drawable, EQ_gc,
            //                  spectrum_x[i
            //                  - 1], spectrum_y[i - 1],
            //                                        spectrum_x[i],
            //                                        spectrum_y[i]);
...
void hdeq_curve_button_press(GdkEventButton* event) {
    ...
    case 2:
        ...
        /*  End drawing - combine the drawn data with any parts of the
            ...
            for (i = 0; i < EQ_input_points; i++) {
                ...
                EQ_yinput[i] =
                    (((((double)EQ_curve_height - (double)EQ_yinput[i]) /
                       (double)EQ_curve_height) *
                      EQ_curve_range_y) +
                     EQ_gain_lower) *
                    0.05;
            ...
            interpolate(EQ_interval, j, EQ_start, EQ_end, &EQ_length, x, y,
                        EQ_xinterp, EQ_yinterp);
    ...
    case 3:
        gtk_menu_popup(HDEQ_menu, NULL, NULL, NULL, NULL, event->button,
...
void hdeq_popup(int action) {
    ...
    /*  If we're drawing something "Cancel" will discard it.  If we're not
     * drawing
     * it does nothing.  */
...
void hdeq_set_upper_gain(float gain) {
    ...
    l_low2mid_adj = gtk_range_get_adjustment((GtkRange*)l_low2mid);
    EQ_curve_range_x = gtk_adjustment_get_lower(l_low2mid_adj) -
                       gtk_adjustment_get_lower(l_low2mid_adj);
...
float hdeq_get_upper_gain() { return (EQ_gain_upper); }
...
void draw_comp_curve(int i, cairo_t* cr) {
    ...
    for (j = NINT(comp_start_x[i]); j <= NINT(comp_end_x[i]); j++) {
        if (!(j % 10)) {
            x1 = NINT(((float)(j - comp_start_x[i]) / comp_curve_range_x[i]) *
                      comp_curve_width[i]);

            //            gdk_draw_line (comp_drawable[i], comp_gc[i], x1, 0,
            //            x1,
            //                comp_curve_height[i]);
    ...
    for (j = NINT(comp_start_y[i]); j <= NINT(comp_end_y[i]); j++) {
        if (!(j % 10)) {
            if (!j) {
                //               gdk_gc_set_line_attributes (comp_gc[i], 2,
                //               GDK_LINE_SOLID,
                //                   GDK_CAP_BUTT, GDK_JOIN_MITER);
                cairo_set_line_width(cr, 2.0);
            } else {
                //               gdk_gc_set_line_attributes (comp_gc[i], 1,
                //               GDK_LINE_SOLID,
                //                   GDK_CAP_BUTT, GDK_JOIN_MITER);
                cairo_set_line_width(cr, 1.0);
            }

            y1 = comp_curve_height[i] -
                 NINT(((float)(j - comp_start_y[i]) / comp_curve_range_y[i]) *
                      comp_curve_height[i]);
    ...
    for (x = comp_start_x[i]; x <= comp_end_x[i]; x += 0.5f) {
        ...
        y1 = comp_curve_height[i] -
             NINT(((y - comp_start_y[i]) / comp_curve_range_y[i]) *
                  comp_curve_height[i]);

        if (x0 != 999.0) {
            //           gdk_draw_line (comp_drawable[i], comp_gc[i], x0, y0,
            //           x1, y1);
...
void comp_curve_box_motion(int i, GdkEventMotion* event) {
    ...
    x = comp_start_x[i] + (((float)event->x / (float)comp_curve_width[i]) *
                           comp_curve_range_x[i]);

    y = comp_start_y[i] + ((((float)comp_curve_height[i] - (float)event->y) /
                            (float)comp_curve_height[i]) *
                           comp_curve_range_y[i]);
...
void comp_box_leave(int i) {
    /* TODO: write the css replacement for this
    gtk_widget_override_color ((GtkWidget *) l_comp_lbl[i],
    GTK_STATE_FLAG_NORMAL,
                               get_color (TEXT_COLOR));
    gtk_widget_override_color ((GtkWidget *) l_c_curve_lbl[i],
    GTK_STATE_FLAG_NORMAL,
                               get_color (TEXT_COLOR)); */
}
...
void hdeq_notebook1_set_page(guint page_num) { notebook1_page = page_num; }

/*  Return the current notebook page - 0 = hdeq, 1 = geq, 2 = spectrum,
    3 = options.  */

int get_current_notebook1_page() { return (notebook1_page); }
