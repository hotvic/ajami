...
void s_suppress_push() { suppress_feedback++; }

void s_suppress_pop() { suppress_feedback--; }
...
void s_save_session(const gchar* fname) {
    ...
    if (!s_have_session_filename()) {
        errstr =
            g_strdup_printf("No session filename found at %s:%d, not saving\n",
                            __FILE__, __LINE__);
    ...
    rootnode =
        xmlNewDocRawNode(doc, NULL, (const xmlChar*)"jam-param-list", NULL);
    ...
    s_save_global_int(doc, "comp bypass0",
                      process_get_bypass_state(LOW_COMP_BYPASS));
    s_save_global_int(doc, "comp bypass1",
                      process_get_bypass_state(MID_COMP_BYPASS));
    s_save_global_int(doc, "comp bypass2",
                      process_get_bypass_state(HIGH_COMP_BYPASS));
    s_save_global_int(doc, "limiter bypass",
                      process_get_bypass_state(LIMITER_BYPASS));

    s_save_global_int(doc, "output meter peak pref",
                      (int)intrim_get_out_meter_peak_pref());
    s_save_global_int(doc, "rms meter peak pref",
                      (int)intrim_get_rms_meter_peak_pref());
    ...
    s_save_global_float(doc, "low delay time",
                        process_get_xo_delay_time(XO_LOW));
    s_save_global_int(doc, "low delay state",
                      process_get_xo_delay_state(XO_LOW));
    s_save_global_float(doc, "mid delay time",
                        process_get_xo_delay_time(XO_MID));
    s_save_global_int(doc, "mid delay state",
                      process_get_xo_delay_state(XO_MID));
...
void s_load_session(const gchar* fname) {
    ...
    if (!s_have_session_filename()) {
        s_set_session_filename(ajami_default_session);
    }
    ...
    for (i = 0; i < XO_NBANDS; i++) {
        if (gp.comp_bypass[i] != 2) {
            /* TODO: callbacks_set_comp_bypass_button_state (i, FALSE); */
        } else {
            /* TODO: callbacks_set_comp_bypass_button_state (i, TRUE); */
        }
    }

    /* TODO: callbacks_set_eq_bypass_button_state (gp.eq_bypass); */
    /* TODO: callbacks_set_limiter_bypass_button_state (gp.limiter_bypass); */
    ...
    /* TODO: callbacks_set_low_delay_button_state (gp.xo_delay_state[XO_LOW]);
     */
    /* TODO: callbacks_set_mid_delay_button_state (gp.xo_delay_state[XO_MID]);
     */
    ...
    if (!override_limiter_default)
        process_set_limiter_plugin(gp.limiter_plugin);
...
void s_startElement(void* user_data, const xmlChar* name,
    ...
    if (!found) {
        ...
        /*  We need to disregard stereo-balance settings as these have been
           removed
...
int s_have_session_filename() { return (session_filename != NULL); }

gchar* s_get_session_filename() { return ((gchar*)session_filename); }

void s_update_title() {
    GString* title =
        g_string_new(client_name ? client_name : g_get_application_name());
    char* base;
    gchar* tmp;
...
float s_get_crossfade_time() { return (crossfade_time); }
...
void s_set_override_limiter_default() { override_limiter_default = TRUE; }
